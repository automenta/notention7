...
import { PropertyEditorPopover } from '../../../components/editor/PropertyEditorPopover';
...
vi.mock('../../../components/map/MapPickerModal', () => ({
  MapPickerModal: vi.fn(({ isOpen, onClose }) =>
    isOpen ? <div data-testid="map-picker-modal" onClick={onClose}>Map Picker</div> : null
  ),
}));
...
const defaultOntology: Map<string, OntologyAttribute> = new Map([
  ['status', { type: 'enum', options: ['active', 'inactive'], operators: { real: ['is'], imaginary: ['is-not'] } }],
  ['price', { type: 'number', operators: { real: ['is'], imaginary: ['<', '>', 'between'] } }],
  ['meeting', { type: 'date', operators: { real: ['is'], imaginary: ['>', '<'] } }],
  ['deadline', { type: 'datetime', operators: { real: ['is'], imaginary: [] } }],
  ['location', { type: 'geo', operators: { real: ['is'], imaginary: [] } }],
  ['description', { type: 'string', operators: { real: ['is'], imaginary: ['contains'] } }],
]);
...
describe('PropertyEditorPopover', () => {
  ...
  it('renders two number inputs for "between" operator', () => {
    renderPopover({ key: 'price', operator: 'between', values: '["100", "200"]' });
  ...
  it('calls onSave with updated values when form is submitted', () => {
    ...
    expect(mockOnSave).toHaveBeenCalledWith('widget-1', 'description', 'is', ['updated']);
  });
