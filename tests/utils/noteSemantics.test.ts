...
describe('matchNotes', () => {
    const ontologyIndex = new Map<string, OntologyAttribute>([
        ...
        ['price', {
            type: 'number',
            operators: {real: ['is'], imaginary: ['is', 'is not', 'less than', 'greater than', 'between']}
        }],
        ['deadline', {
            type: 'date',
            operators: {
                real: ['is'],
                imaginary: ['is on', 'is not on', 'is after', 'is before', 'between', 'is not between']
            }
        }],
        ['eventTime', {
            type: 'datetime',
    ...
    describe('Date/DateTime Matching', () => {
        it.each([
            ...
            {
                key: 'deadline',
                sourceVal: '2024-05-15',
                op: 'between',
                queryVal: ['2024-05-10', '2024-05-20'],
                match: true
            },
            {
                key: 'deadline',
                sourceVal: '2024-05-25',
                op: 'is not between',
                queryVal: ['2024-05-10', '2024-05-20'],
                match: true
            },
        ])('should handle date op "$op" on key "$key" correctly (match: $match)', ({
                                                                                       key,
                                                                                       sourceVal,
                                                                                       op,
                                                                                       queryVal,
