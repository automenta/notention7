    ...
    return (
        <div className="h-full flex flex-col bg-gray-800/70">
            <div className="flex-shrink-0 p-3 border-b border-gray-700/50 flex items-center gap-3">
                <button onClick={onBack} className="md:hidden p-2 -ml-1 text-gray-400 hover:text-white"><ArrowLeftIcon
                    className="h-6 w-6"/></button>
                <img
                    src={selectedProfile?.picture || `https://api.dicebear.com/8.x/bottts-neutral/svg?seed=${selectedContact.pubkey}`}
                    className="h-10 w-10 rounded-full bg-gray-700"/>
                <div>
                    <p className="font-bold text-white">{selectedProfile?.name || 'Anonymous'}</p>
                    <p className="text-xs text-gray-400 font-mono"
                       title={nip19.npubEncode(selectedContact.pubkey)}>{formatNpub(nip19.npubEncode(selectedContact.pubkey))}</p>
                </div>
            </div>
            ...
            <div className="flex-shrink-0 p-4">
                <form onSubmit={handleSendMessage} className="flex gap-2">
                    ...
                    <button type="submit" className="p-3 bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-gray-500">
                        <SendIcon className="h-6 w-6"/></button>
                </form>
            </div>
        </div>
    );
