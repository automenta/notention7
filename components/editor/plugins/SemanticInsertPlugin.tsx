...
import { SemanticInsertModal, InsertMenuItem } from '../../editor/SemanticInsertModal';
...
export const SemanticInsertToolbar: React.FC<{ editorApi: EditorApi }> = ({ editorApi }) => {
    return (
        <>
            <div className="w-px h-6 bg-gray-700 mx-1"></div>
            <button onClick={() => editorApi.openSemanticInsertModal('tag')} className={buttonClass} title="Insert Tag">
                <TagIcon className="h-5 w-5" />
            </button>
            <button onClick={() => editorApi.openSemanticInsertModal('template')} className={buttonClass} title="Insert Template">
...
export const SemanticInsertModalProvider: React.FC<{ editorApi: EditorApi }> = ({ editorApi }) => {
    ...
    const handleSelect = (item: InsertMenuItem) => {
        ...
        if (modalState.type === 'tag') {
            ...
            if (template && template.attributes) {
                htmlToInsert = Object.keys(template.attributes).map((key) => {
                    const k = key.trim();
                    const v = [''];
                    const op = 'is';
