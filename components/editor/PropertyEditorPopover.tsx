...
export const PropertyEditorPopover: React.FC<PropertyEditorPopoverProps> = ({
                                                                                widgetEl,
                                                                                onSave,
                                                                                onDelete,
                                                                                onClose,
    ...
    const renderValueInputs = () => {
        ...
        return Array.from({length: numInputs}).map((_, index) => {
            ...
            switch (type) {
                case 'number':
                    return <input key={index} type="number" value={value}
                                  onChange={e => handleValueChange(index, e.target.value)} placeholder={placeholder}
                                  className={inputClass}/>;
                case 'date':
                    return <input key={index} type="date" value={value}
                                  onChange={e => handleValueChange(index, e.target.value)} className={inputClass}/>;
                case 'datetime':
                    const dtValue = value.slice(0, 16); // Format for datetime-local
                    return <input key={index} type="datetime-local" value={dtValue}
                                  onChange={e => handleValueChange(index, e.target.value)} className={inputClass}/>;
                case 'enum':
                    return (
                        <select key={index} value={value} onChange={e => handleValueChange(index, e.target.value)}
                                className={inputClass}>
                ...
                case 'geo':
                    return (
                        <div key={index} className="flex items-center gap-2">
                            <input type="text" placeholder="lat,lng" value={value}
                                   onChange={e => handleValueChange(index, e.target.value)} className={inputClass}/>
                            <button type="button" onClick={() => setIsMapPickerOpen(true)}
                                    className="p-2 mt-1 bg-blue-600 rounded-md hover:bg-blue-700" title="Select on Map">
    ...
    return ReactDOM.createPortal(
        <>
            <div ref={popoverRef} style={popoverStyle}
                 className="bg-gray-800 border border-gray-600 rounded-lg shadow-2xl p-4 text-white w-80 animate-fade-in">
                <form onSubmit={handleSave} className="space-y-4">
                    ...
                    {attributeType && attributeType.operators && (
                        <div>
                            ...
                            <select value={operator} onChange={e => setOperator(e.target.value)} className={inputClass}>
                                {attributeType.operators.real && attributeType.operators.real.length > 0 &&
                                    <optgroup label="Real">{attributeType.operators.real.map(op => <option key={op}
                                                                                                           value={op}>{op}</option>)}</optgroup>}
                                {attributeType.operators.imaginary && attributeType.operators.imaginary.length > 0 &&
                                    <optgroup label="Imaginary">{attributeType.operators.imaginary.map(op => <option
                                        key={op} value={op}>{op}</option>)}</optgroup>}
                            </select>
                        </div>
                    )}
                    ...
                    <div>
                        <label className={labelClass}>
                            ...
                            {attributeType?.type && <span
                                className="font-mono text-xs lowercase px-2 py-0.5 bg-gray-600 rounded">{attributeType.type}</span>}
                        </label>
                    ...
                    <div className="flex justify-between items-center pt-2">
                        <button type="button" onClick={() => onDelete(widgetEl.id)}
                        ...
                        <div className="flex gap-2">
                            <button type="button" onClick={onClose}
                                    className="px-4 py-2 text-sm bg-gray-600 rounded-md hover:bg-gray-500">Cancel
                            </button>
                            <button type="submit"
                                    className="px-4 py-2 text-sm bg-blue-600 rounded-md hover:bg-blue-700">Save
                            </button>
                        </div>
                    </div>
                </form>
