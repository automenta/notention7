...
const inputClass =
  'w-full p-2 mt-1 bg-gray-700 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm';
const labelClass =
  ...
  useEffect(() => {
    ...
    if (newAttributeType) {
      if (newAttributeType.operators) {
        const allOperators = [
          ...newAttributeType.operators.real,
          ...newAttributeType.operators.imaginary,
        ];
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !widgetEl.contains(event.target as Node)
      ) {
        onClose();
      }
    };
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className={labelClass}>
              ...
              {attributeType?.type && (
                <span className="font-mono text-xs lowercase px-2 py-0.5 bg-gray-600 rounded">
                  {attributeType.type}
                </span>
              )}
            </label>
            <div className="space-y-2">{renderValueInputs()}</div>
          </div>

          <div className="flex justify-between items-center pt-2">
            <button
              type="button"
              onClick={() => onDelete(widgetEl.id)}
